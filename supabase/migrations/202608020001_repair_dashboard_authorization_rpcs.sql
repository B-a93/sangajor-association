...
-- Always expose the dashboard RPC, even on production databases where the optional
-- communication tables have not been installed. Dynamic SQL prevents function
-- validation from introducing a dependency on those tables.
...
begin
  if not public.is_active_member(auth.uid())
     ...
     or to_regclass('public.announcement_reads') is null then
    return 0;
  end if;
